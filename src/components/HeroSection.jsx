import { ArrowDown } from "lucide-react";

export const HeroSection = () => {
  return (
    <section
      id="hero"
      className="relative min-h-screen flex flex-col items-center justify-center px-4 bg-cover bg-center"
      style={{ backgroundImage: "url('hero-cover.jpg')" }}
    >
      {/* translucent overlay */}
      <div className="absolute inset-0 bg-black/35"></div>

      <div className="container max-w-4xl mx-auto text-center z-10 relative">
        <div className="space-y-6">
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight">
            <span className="text-background opacity-0 animate-fade-in">
              Clare's
            </span>
            <span className="text-primary opacity-0 animate-fade-in-delay-1">
              {" "}
              UNGCNS
            </span>
            <span className="text-background text-gradient ml-2 opacity-0 animate-fade-in-delay-2">
              {" "}
              Internship
            </span>
          </h1>

          <p className="text-lg md:text-xl text-background">
            A three-month journey in sustainability, digital communications, and
            global stakeholder engagement at the United Nations Global Compact
            Network Singapore.
          </p>

          <div className="pt-4 opacity-0 animate-fade-in-delay-4">
            <a href="#about" className="cosmic-button">
              Explore My Experience
            </a>
          </div>
        </div>
      </div>

      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex flex-col items-center animate-bounce z-10">
        <span className="text-sm text-muted-foreground mb-2">Scroll</span>
        <ArrowDown className="h-5 w-5 text-primary" />
      </div>
    </section>
  );
};
